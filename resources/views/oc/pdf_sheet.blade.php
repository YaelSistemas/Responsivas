{{-- resources/views/oc/pdf_sheet.blade.php --}}
@php
  ...
  /* ===== Empresa / Logo (misma lógica que show) ===== */
  $empresaId     = (int) session('empresa_activa', auth()->user()?->empresa_id);
  $empresaNombre = config('app.name', 'Laravel');
  $logoFile      = public_path('images/logos/default.png'); // fallback absoluto
  ...
  if (class_exists(\App\Models\Empresa::class) && $empresaId) {
    ...
    if ($emp) {
      ...
      foreach ($candidates as $abs) {
        if ($abs && is_file($abs)) { $logoFile = $abs; break; }
      }
    }
  }
  ...
  $baseBlocks   = 9;
  $blocksToDraw = max($baseBlocks, $detalles->count()); // seguimos dibujando todo

  /* ===== Layout adaptativo para forzar 1 sola hoja ===== */
  $rows = $detalles->count();

  // Valores por defecto (coinciden con tu show)
  $PAGE_MARGIN = '10mm';
  $FS_BASE     = 12;   // font-size base de celdas
  $PAD         = 6;    // padding de celdas
  $ROW_H       = 26;   // altura de fila data
  $SPACER_H    = 12;   // altura de spacer entre filas
  $HERO_ROW_H  = 36;   // altura por fila del header
  $LOGO_MAX_H  = 105;  // alto máx de logo
  $LOGO_MAX_W  = 220;  // ancho máx de logo
  $TITLE_MAIN  = 14;   // h1
  $TITLE_SUB   = 12;   // h2/etiquetas
...
<head>
  ...
  <style>
    ...
    /* ===== Encabezado ===== */
    .hero { table-layout: fixed; --row-h: {{ $HERO_ROW_H }}px; }
    ...
    .title-main{ font-weight:800; font-size: {{ $TITLE_MAIN }}px; }
    .title-sub{ font-weight:700; font-size: {{ $TITLE_SUB }}px; text-transform:uppercase; letter-spacing:.3px; }
    ...
    /* -- Bordes especiales -- */
    /* Columna 1 */
    .prov-f2 { border-right:0 !important; border-bottom:0 !important; } /* fila 2 */
    .prov-f3, .prov-f4 { border-top:0 !important; border-right:0 !important; border-bottom:0 !important; } /* filas 3 y 4 */
    .prov-f5 { border-top:0 !important; border-right:0 !important; } /* fila 5 */

    /* Columna 2 */
    .prov-c2-r2 { border-left:0 !important; border-bottom:0 !important; } /* fila 2 */
    .prov-c2-r3, .prov-c2-r4 { border-top:0 !important; border-left:0 !important; border-bottom:0 !important; } /* filas 3 y 4 */
    .prov-c2-r5 { border-top:0 !important; border-left:0 !important; } /* fila 5 */

    /* ===== Items ===== */
    .items { margin-top: {{ max(8,$PAD+2) }}px; }
    ...
    .items td.money > .mwrap, .items td.money-total > .mwrap {
      display:flex; align-items:center; justify-content:space-between; width:100%; gap:6px;
    }
    .items td.money > .mwrap .sym, .items td.money-total > .mwrap .sym {
      flex:0 0 auto; min-width:1.2em; text-align:left;
    }
    .items td.money > .mwrap .val, .items td.money-total > .mwrap .val {
      flex:1 1 auto; text-align:right; font-variant-numeric: tabular-nums;
    }
    ...
    /* Imagen del pie (ocupa col 4-5, filas 6–8) */
    .footer-img-td{
      background-image: url('{{ $footerB64 }}');
      background-repeat:no-repeat; background-position:center center; background-size:contain;
      height: {{ max(90, $ROW_H*4) }}px;
    ...
    /* Si estamos en modo ultra (sin spacers) escondemos la fila spacer */
    @if (!$DRAW_SPACERS)
...
<body>
  ...
  <table class="tbl hero">
    <colgroup>
      <col style="width:28%"><col style="width:52%"><col style="width:20%">
    </colgroup>
    <tr>
  ...
  <table class="tbl meta-grid upper" style="margin-top:8px">
    ...
    <tr>
      ...
      <td rowspan="2" class="order-no center">{{ $oc->numero_orden ?? '' }}</td>
    </tr>
    <tr>
      <td class="prov-f3">&nbsp;</td>
  ...
  <table class="tbl items upper" style="margin-top:12px">
    <colgroup>
      <col style="width:10%"><col style="width:10%"><col style="width:48%"><col style="width:12%"><col style="width:20%">
    </colgroup>
    ...
    @for ($bloque = 0; $bloque < $blocksToDraw; $bloque++)
      ...
      <tr class="data">
        ...
        <td>{{ $d ? ($concepto ?: ' ') : ' ' }}</td>
        <td class="money mono">
          <div class="mwrap"><span class="sym">{{ $pSym ?: ' ' }}</span><span class="val">{{ $pVal ?: ' ' }}</span></div>
        </td>
        <td class="money mono">
          <div class="mwrap"><span class="sym">{{ $iSym ?: ' ' }}</span><span class="val">{{ $iVal ?: ' ' }}</span></div>
        </td>
      </tr>
    ...
    {{-- ===== 8 filas extra ===== --}}
    {{-- NOTA: .no-upper evita heredar mayúsculas; solo el nombre del usuario va en mayúsculas con .upper-force --}}
    ...
    <tr class="extra e6 no-upper">
      <td colspan="2">&nbsp;</td>
      <td class="xxsmall">LAS FACTURAS DEBERAN MOSTRAR EL NUMERO DE ESTE ORDEN PARA SER PAGADAS</td>
