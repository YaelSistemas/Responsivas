...
<x-app-layout title="Proveedores">
  <x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight text-center">Proveedores</h2>
  </x-slot>

  <style>
    /* ====== Zoom responsivo (igual que OC) ====== */
    ...
    .tbl col.c-nom   { width:28% }
    .tbl col.c-dir   { width:42% }  /* calle, colonia, CP */
    .tbl col.c-ubi   { width:24% }  /* ciudad, estado */
    .tbl col.c-acc   { width:6%  }

    /* Toolbar */
    #pv-toolbar .select-wrap{position:relative;display:inline-block}
    #pv-toolbar select[name="per_page"]{
  ...
  <div class="zoom-outer">
    <div class="zoom-inner">
      <div class="page-wrap py-6">
        ...
        {{-- Toolbar --}}
        <form id="pv-toolbar" method="GET" action="{{ route('proveedores.index') }}" class="mb-3">
          <div class="flex items-center justify-between gap-3">
            ...
            <div class="text-sm text-gray-700 flex items-center gap-2">
              ...
              <input id="q" name="q" value="{{ $q ?? '' }}" autocomplete="off"
                     class="border rounded px-3 py-1 w-80 focus:outline-none"
                     placeholder="Nombre, RFC, ciudad, estado…">
            </div>
          </div>
        </form>
        ...
        @if (session('created') || session('updated') || session('deleted') || session('error'))
          @php
            $msg = session('created') ? 'Proveedor creado.'
                 : (session('updated') ? 'Proveedor actualizado.'
                 : (session('deleted') ? 'Proveedor eliminado.'
                 : (session('error') ?: '')));
        ...
        {{-- Tabla (parcial) --}}
        <div class="card">
          <div class="overflow-x-auto" id="pv-wrap">
            @include('proveedores.partials.table', ['proveedores' => $proveedores])
          </div>
        </div>
      </div>
    </div>
  ...
  {{-- AJAX: búsqueda / per_page / paginación (clonado de OC) --}}
  <script>
  (function(){
    const input = document.getElementById('q');
    const perPageSelect = document.querySelector('#pv-toolbar select[name="per_page"]');
    const wrap = document.getElementById('pv-wrap');
    let t, ctl;

    function buildUrl(pageUrl = null){
      const base = pageUrl || "{{ route('proveedores.index') }}";
