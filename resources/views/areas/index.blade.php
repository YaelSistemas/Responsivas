<x-app-layout title="Áreas">
  <x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight text-center">Áreas</h2>
  </x-slot>
  ...
  <style>
    ...
    #areas-toolbar .select-wrap .caret{
  ...
  (function(){
    ...
    const perPageSelect = document.querySelector('#areas-toolbar select[name="per_page"]');
    const wrap = document.getElementById('areas-wrap');
    let t, ctl;

    function buildUrl(pageUrl = null){
      const base = pageUrl || "{{ route('areas.index') }}";
