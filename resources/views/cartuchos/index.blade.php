...
<x-app-layout title="Cartuchos">
  ...
  <style>
    ...
    /* SKU puede romper línea (opcional) */
    .tbl td:nth-child(6){
    white-space:normal;
    overflow:visible;
    text-overflow:unset;
    ...
    /* Anchos de columnas */
    .tbl col.c-folio   { width:10% }
    .tbl col.c-fecha   { width:12% }
    .tbl col.c-colab   { width:18% }
    .tbl col.c-unidad  { width:14% }
    .tbl col.c-equipo  { width:18% }
    .tbl col.c-sku     { width:12% }
    .tbl col.c-realizo { width:12% }
    .tbl col.c-acc     { width:10% }

    /* Toolbar */
    #cart-toolbar .select-wrap{position:relative;display:inline-block}
    #cart-toolbar select[name="per_page"]{
    ...
    #cart-toolbar .select-wrap .caret{
  ...
  <div class="zoom-outer">
    <div class="zoom-inner">
      <div class="page-wrap py-6">
        ...
        {{-- Toolbar --}}
        <form id="cart-toolbar" method="GET" action="{{ route('cartuchos.index') }}" class="mb-3">
          <div class="flex items-center justify-between gap-3">
            ...
            <div class="text-sm text-gray-700 flex items-center gap-2">
              ...
              <input id="q" name="q" value="{{ $q ?? '' }}" autocomplete="off"
                    ...
                    placeholder="Folio, colaborador, producto o realizó">
            </div>
          </div>
        </form>
        ...
        @if (session('created') || session('updated') || session('deleted') || session('error'))
          @php
            $msg = session('created') ? 'Cartucho creado.'
                 : (session('updated') ? 'Cartucho actualizado.'
                 : (session('deleted') ? 'Cartucho eliminado.'
                 : (session('error') ?: '')));
        ...
        {{-- Tabla (parcial) --}}
        <div class="card">
          <div class="overflow-x-auto" id="cart-wrap">
            @include('cartuchos.partials.table')
          </div>
        </div>
      </div>
    </div>
  ...
  (function(){
    ...
    const perPageSelect = document.querySelector('#cart-toolbar select[name="per_page"]');
    const wrap = document.getElementById('cart-wrap');
    let t, ctl;

    function buildUrl(pageUrl = null){
      const base = pageUrl || "{{ route('cartuchos.index') }}";
