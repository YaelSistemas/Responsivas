...
<div class="colab-modal-backdrop" data-modal-backdrop>
  <div class="colab-modal" role="dialog" aria-modal="true" aria-label="Historial de la subsidiaria">
    <header>
      <div>
        <div class="ev-title">Historial — {{ $subsidiaria->nombre }}</div>
        <div class="ev-meta">ID #{{ $subsidiaria->id }}</div>
      </div>
    ...
    <div class="content">
      @if($historial->isEmpty())
        <p class="ev-meta">Sin historial registrado para esta subsidiaria.</p>
      @else
        <ul class="timeline">
          @foreach($historial as $log)
            @php
              ...
              // 🔹 Filtramos solo los campos relevantes (nombre y descripción)
              $otrosCambios = collect($cambios)->only(['nombre', 'descripcion']);
            @endphp
