<x-app-layout title="Editar Subsidiaria">
  <x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight text-center">Editar Subsidiaria</h2>
  </x-slot>
  ...
  <style>
    ...
    /* Breakpoints (mismos que en demás vistas) */
    ...
    <div class="zoom-inner">
      <div class="py-6 page-pad">
        <div class="form-container">
          ...
          <form method="POST" action="{{ route('subsidiarias.update', $subsidiaria) }}">
            @csrf
            @method('PUT')

            <div class="form-group">
              <label>Nombre</label>
              <input name="nombre" value="{{ old('nombre', $subsidiaria->nombre) }}" required>
              @error('nombre') <p style="color:#dc2626;margin-top:6px">{{ $message }}</p> @enderror
            </div>

            <div class="form-group">
              <label>Razón Social (opcional)</label>
              <textarea name="descripcion" rows="3">{{ old('descripcion', $subsidiaria->descripcion) }}</textarea>
