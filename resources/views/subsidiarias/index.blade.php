<x-app-layout title="Subsidiarias">
  <x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight text-center">Subsidiarias</h2>
  </x-slot>
  ...
  <style>
    ...
    #subs-toolbar .select-wrap .caret{
    ...
    <div class="zoom-inner">
      <div class="page-wrap py-6">
        ...
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-xl font-semibold">Subsidiarias</h2>
          @if($canCreate)
        ...
        {{-- Toolbar --}}
        <form id="subs-toolbar" method="GET" action="{{ route('subsidiarias.index') }}" class="mb-3">
        ...
        @if (session('created') || session('updated') || session('deleted') || session('error'))
          @php
            $msg = session('created') ? 'Subsidiaria creada.'
                 : (session('updated') ? 'Subsidiaria actualizada.'
                 : (session('deleted') ? 'Subsidiaria eliminada.'
                 : (session('error') ?: '')));
  ...
  (function(){
    ...
    const perPageSelect = document.querySelector('#subs-toolbar select[name="per_page"]');
    const wrap = document.getElementById('subs-wrap');
    let t, ctl;

    function buildUrl(pageUrl = null){
      const base = pageUrl || "{{ route('subsidiarias.index') }}";
