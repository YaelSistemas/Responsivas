<x-app-layout title="Devoluciones">
  <x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight text-center">Devoluciones</h2>
  </x-slot>
  ...
  <style>
    ...
    /* Colaborador a una línea */
    .tbl td:nth-child(2){ white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }

    /* Productos devueltos permite salto */
    .tbl td:nth-child(6){
      white-space:normal;
      overflow:visible;
      text-overflow:unset;
    ...
    .tbl col.c-colab   { width:18% }
    .tbl col.c-area    { width:9%  }
    .tbl col.c-motivo  { width:9%  }
    .tbl col.c-resp    { width:9%  }
    .tbl col.c-prod    { width:20% }
    .tbl col.c-fdev    { width:9%  }
    .tbl col.c-recibio { width:9%  }
    .tbl col.c-acc     { width:5%  }

    /* Toolbar */
    #dev-toolbar .select-wrap{position:relative;display:inline-block}
    #dev-toolbar select[name="per_page"]{
    ...
    <div class="zoom-inner">
      <div class="page-wrap py-6">
        ...
        <div class="flex items-center justify-between mb-4">
          ...
          <a href="{{ route('devoluciones.create') }}" class="btn btn-primary">+ Nueva</a>
        </div>

        {{-- Toolbar --}}
        <form id="dev-toolbar" method="GET" action="{{ route('devoluciones.index') }}" class="mb-3">
          <div class="flex items-center justify-between gap-3">
            ...
            <div class="text-sm text-gray-700 flex items-center gap-2">
              ...
              <input id="q" name="q" value="{{ $q ?? '' }}" autocomplete="off"
                     ...
                     placeholder="Folio o colaborador">
            </div>
          </div>
        </form>
        ...
        {{-- Tabla --}}
        <div class="card">
          <div class="overflow-x-auto" id="dev-wrap">
            @include('devoluciones.partials.table')
          </div>
        </div>
      </div>
    </div>
  ...
  (function(){
    ...
    const perPageSelect = document.querySelector('#dev-toolbar select[name="per_page"]');
    const wrap = document.getElementById('dev-wrap');
    let t, ctl;

    function buildUrl(pageUrl = null){
      const base = pageUrl || "{{ route('devoluciones.index') }}";
