<x-app-layout title="Editar Puesto">
  <x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight text-center">Editar puesto</h2>
  </x-slot>
  ...
  <style>
    ...
    /* Breakpoints (consistente con tus otras vistas) */
        ...
        <div class="form-container">
          ...
          <form method="POST" action="{{ route('puestos.update', $puesto) }}">
            ...
            <div class="form-group">
              <label>Nombre</label>
              <input name="nombre" value="{{ old('nombre', $puesto->nombre) }}" required>
              @error('nombre') <p style="color:#dc2626;margin-top:6px">{{ $message }}</p> @enderror
            </div>

            <div class="form-group">
              <label>Descripción (opcional)</label>
              <textarea name="descripcion" rows="3">{{ old('descripcion', $puesto->descripcion) }}</textarea>
