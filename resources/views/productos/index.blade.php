<x-app-layout title="Productos">
  <x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight text-center">Productos</h2>
  </x-slot>
  ...
  (function(){
    ...
    const perPageSelect = document.querySelector('#prod-toolbar select[name="per_page"]');
    const wrap = document.getElementById('prod-wrap');
    let t, ctl;

    function buildUrl(pageUrl = null){
      const base = pageUrl || "{{ route('productos.index') }}";
